...
# extra kawaii/deco styles
def sparkle_deco(text): return f"✧･ﾟ: *✧ {text.lower()} ✧*:･ﾟ✧"
def kaomoji_infused(text): return f"{text} (≧◡≦) (ฅ^•ﻌ•^ฅ)"
def soft_gradient_fake(text): return "🌷" + "͟".join(list(text)) + "🌷"
def kawaii_kana_hybrid(text): return text.translate(str.maketrans("aeoy", "αєσү"))
def emoji_border(text): return f"💌💌 {text.lower()} 💌💌"
def tiny_spell(text): return ''.join((c + '\u033d\u0343') if c.isalpha() else c for c in text)
def supersoft_sans(text): return to_sans_bold(text)
def pixel_retro(text):
    upper = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    squared = "🄰🄱🄲🄳🄴🄵🄶🄷🄸🄹🄺🄻🄼🄽🄾🄿🅀🅁🅂🅃🅄🅅🅆🅇🅈🅉"
    table = {ord(a): b for a, b in zip(upper, squared)}
    table.update({ord(a.lower()): b for a, b in zip(upper, squared)})
    return text.translate(table)
def handwritten_doodle(text): return to_script(text) + " ✏️💭"
def pastel_icon_injections(text):
    out = []
...
kawaii_styles = [
    ("Sparkle Deco Text", sparkle_deco, ["✧","･ﾟ:","*✧"], "✧"),
    ("Kaomoji-Infused", kaomoji_infused, ["✨","🌸","💖"], "⭐"),
    ("Soft Gradient Look", soft_gradient_fake, ["🌷","✨","🌷"], "🌸"),
    ("Kawaii Kana Hybrid", kawaii_kana_hybrid, ["💖","🌸","✨"], "⭐"),
    ("Emoji Bordering", emoji_border, ["💌","💌","💌"], "💌"),
    ("Tiny Above/Below Decorations", tiny_spell, ["✨","💫","✨"], "💫"),
    ("Supersoft Sans", supersoft_sans, ["⭐","✨","⭐"], "🌸"),
    ("Pixel / Retro", pixel_retro, ["🕹️","🎮","🕹️"], "⭐"),
    ("Handwritten Doodle", handwritten_doodle, ["📝","💭","🌸"], "💖"),
    ("Pastel Icon Injections", pastel_icon_injections, ["💖","✨","💖"], "🌸"),
]

# UI
user_text = st.text_input("Enter text:", "thank you gary")
handles = "@higary__ @garyJapan__"  # <-- both handles now

st.write("### Fonts")
st.write("---")
for name, fn, prefix_emojis, suffix_emoji in base_styles:
    styled = fn(user_text)
    left = " ".join(prefix_emojis)
    output = f"{left} {styled} {handles} {suffix_emoji}"
    st.markdown(f"**{name}**")
    st.markdown(f"<div style='font-size:1.6em; padding:6px 0'>{output}</div>", unsafe_allow_html=True)

st.write("### Kawaii / Deco Styles")
st.write("---")
for name, fn, prefix_emojis, suffix_emoji in kawaii_styles:
    styled = fn(user_text)
    left = " ".join(prefix_emojis)
    output = f"{left} {styled} {handles} {suffix_emoji}"
    st.markdown(f"**{name}**")
